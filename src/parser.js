...
export const parse = (raw) => {
  ...
  for (const rawLine of lines) {
    ...
    // Heading detection requires NO leading whitespace — that's the
    // convention used by welcome.txt to mark '# order: 1' as literal text
    // instead of as a heading.
    if (line.startsWith('## ') && !/^[ \t]/.test(rawLine)) {
      flushPara();
      flushList();
    ...
    flushList();
    if (/^[ \t]/.test(rawLine) || /^#[^ \t]/.test(line)) paraLiteral = true;
    para.push(line);
  }
  flushPara();
  flushList();
